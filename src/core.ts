...
function cmFunctionInsertText(func: string): string {
  ...
  const is_scoped = scoped_func.reduceRight(
    (prev, name) => prev || func == name,
    false
  )
  if (is_scoped) {
    return func + '(${1})\n\t\nend' + func + '(${1})\n'
...
export function cmCommandsSuggestions(
  currentWord: string
): Thenable<CompletionItem[]> {
...
export function cmVariablesSuggestions(
  currentWord: string
): Thenable<CompletionItem[]> {
...
export function cmPropertiesSuggestions(
  currentWord: string
): Thenable<CompletionItem[]> {
...
export function cmModulesSuggestions(
  currentWord: string
): Thenable<CompletionItem[]> {
...
export function cmCommandsSuggestionsExact(
  currentWord: string
): Thenable<CompletionItem[]> {
  const cmd = cmake_help_command_list()
  return suggestionsHelper(
...
export function cmVariablesSuggestionsExact(
  currentWord: string
): Thenable<CompletionItem[]> {
  const cmd = cmake_help_variable_list()
  return suggestionsHelper(
...
export function cmPropertiesSuggestionsExact(
  currentWord: string
): Thenable<CompletionItem[]> {
  const cmd = cmake_help_property_list()
  return suggestionsHelper(
...
export function cmModulesSuggestionsExact(
  currentWord: string
): Thenable<CompletionItem[]> {
  const cmd = cmake_help_module_list()
